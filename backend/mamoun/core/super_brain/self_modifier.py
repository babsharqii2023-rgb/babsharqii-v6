"""
Self Modifier v58 — Safe self-modification with REAL sandbox and post-apply verification.

CRITICAL UPGRADE from v57:
- v57: Quality score was hardcoded (0.9 for low risk, 0.7 for medium)
- v58: Quality score based on actual verification results
- Added post-apply verification: test the modified code AFTER writing
- Better rollback with verification
- Added modification history tracking
- Fixed quality scoring to reflect real verification outcomes
...
class ASTSecurityChecker:
    ...
    ALLOWED_OS_ATTRS = {"path", "environ", "getcwd", "listdir", "makedirs", "path"}
    ALLOWED_OS_FROM = {"os.path"}
    ...
    def execute(self, code: str, timeout: int = 30, context: dict = None) -> dict:
        ...
        if not is_safe:
            return {
                ...
                "error": f"Security violations: {violations}", "output": "",
            }
...
class SelfModifier:
    ...
    4. POST-CHECK: Verify the applied code still works
    5. ROLLBACK: If post-check fails, automatically revert
    ...
    async def verify(self, proposal: ModificationProposal) -> dict:
        """Verify a proposed modification through multiple checks."""
        results = {
            "ast_check": {"passed": False, "violations": []},
            "syntax_check": {"passed": False},
            "sandbox_execution": {"passed": False, "error": None},
            "llm_review": {"passed": True, "concerns": []},
            "overall": False,
            "quality_score": 0.0,  # Real quality based on verification
        }
        ...
        # Compute quality from verification results
        checks_passed = sum(1 for k in ["ast_check", "syntax_check", "sandbox_execution"]
                          if results[k]["passed"])
        total_checks = 3
        quality = checks_passed / total_checks

        if not sandbox_result["success"]:
            results["overall"] = False
            results["quality_score"] = quality * 0.5  # Partial credit
            return results

        # 4. LLM Review (for high-risk changes)
        if proposal.risk_level == "high" and self._llm_client:
            try:
                ...
                if llm_response.success:
                    ...
                    if concerns:
                        quality *= 0.7  # Reduce quality for concerns
        ...
        results["quality_score"] = quality if results["overall"] else quality * 0.5

        return results

    # ── Phase 2: Apply ───────────────────────────────────────────────────
    async def apply(self, proposal: ModificationProposal) -> dict:
        """Apply a verified modification with snapshot for rollback."""
    ...
    async def post_check(self, proposal: ModificationProposal) -> dict:
        """Verify that the applied modification still works after writing."""
        try:
            ...
            # Re-run sandbox
            sandbox_result = self._executor.execute(applied_code, timeout=15)

            post_check_passed = sandbox_result["success"]
            return {
            ...
            return {"passed": False, "error": f"Post-apply syntax error: {e}"}
        except FileNotFoundError:
            return {"passed": False, "error": "File not found after apply"}
        except Exception as e:
            return {"passed": False, "error": f"Post-check error: {e}"}
    ...
    async def modify(self, proposal: ModificationProposal) -> dict:
        """Full modification pipeline: verify → apply → post-check → (rollback if needed)."""
        start = time.time()
        ...
        if not verification["overall"]:
            ...
            return {"success": False, "phase": "verification", "details": verification}
        ...
        if not apply_result["success"]:
            return {"success": False, "phase": "application", "details": apply_result}

        # Phase 3: Post-check (NEW in v58)
        proposal.status = ModificationStatus.POST_CHECKING
        post_check = await self.post_check(proposal)
        proposal.post_check_result = post_check

        if not post_check["passed"]:
            # Post-check failed — rollback
            logger.warning(f"Post-apply check failed for {proposal.id}, rolling back")
            await self.rollback(proposal)
            return {
                "success": False,
                "phase": "post_check",
                "details": {"post_check": post_check, "rollback": "auto"},
            }
        ...
        if self._meta_cognition:
            try:
                ...
                self._meta_cognition.record_outcome(OutcomeRecord(
                    ...
                    metadata={"target": proposal.target_file, "risk": proposal.risk_level, "post_check": "passed"},
                ))
            except ImportError:
                pass
