...
class TestVersionConsistency:
    ...
    def test_kernel_version(self):
        """MamounKernel should report v59.1."""
        from mamoun.core.super_brain.mamoun_kernel import MamounKernel
        kernel = MamounKernel()
        status = kernel.get_status()
        assert status["version"] == "v59.1", f"Expected v59.1, got {status['version']}"

    def test_kernel_self_assessment_version(self):
        """Kernel self-assessment should report v59.1."""
        from mamoun.core.super_brain.mamoun_kernel import MamounKernel
        kernel = MamounKernel()
        assessment = kernel.get_self_assessment()
        assert assessment["version"] == "v59.1", f"Expected v59.1, got {assessment['version']}"
